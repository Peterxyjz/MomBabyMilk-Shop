    ...
    <div style={{ display: 'flex', justifyContent: 'center', height: '80vh' }}>
      <Card title="Đơn đã xác nhận" style={{ width: '90%', marginTop: '50px', height: '75vh' }}>
        <div>
          ...
          <Table
            dataSource={orders.filter(item => item.order.status === 1)}
            rowKey={item => item.order._id}
            pagination={{
              current: currentPage,
              pageSize: pageSize,
              total: orders.filter(item => item.order.status === 1).length,
          ...
          >
            ...
            <Column
              title="Trạng Thái"
              key="status"
              render={() => <Tag icon={<CheckCircleOutlined />} bordered={false} color="processing">Đã Xác Nhận</Tag>}
            />
            <Column
