    ...
    <div style={{ display: 'flex', justifyContent: 'center', height: '80vh' }}>
      <Card title="Đơn đã hủy" style={{ width: '90%', marginTop: '50px', height: '75vh' }}>
        <div>
          ...
          <Table
            dataSource={orders.filter(item => item.order.status === 3)}
            rowKey={item => item.order._id}
            pagination={{
              current: currentPage,
              pageSize: pageSize,
              total: orders.filter(item => item.order.status === 3).length,
          ...
          >
            ...
            <Column
              title="Trạng Thái"
              key="status"
              render={() => <Tag icon={<CloseCircleOutlined />} bordered={false} color="error">Đã Hủy</Tag>}
            />
            <Column
